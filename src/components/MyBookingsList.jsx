  ...
  if (bookings.length === 0) {
    return (
      <div className="text-center py-20">
        ...
        <Button className="btn-premium text-white hover:scale-105 transition-transform duration-300">Browse Services</Button>
      </div>
    );
  }
  ...
  return (
    <div className="grid gap-4">
      {bookings.map((booking) => {
        ...
        return (
          <Card key={booking.id} className="glass-card border-white/10 hover:border-white/20 transition-all duration-300 hover:-translate-y-1">
            <CardContent className="p-6">
              <div className="flex flex-col md:flex-row md:items-center gap-4">
                ...
                <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div><div className="text-sm text-gray-400">Service</div><div className="text-white font-medium">{booking.service}</div></div>
                  <div><div className="text-sm text-gray-400 flex items-center gap-1"><Clock className="h-3 w-3" />Duration</div><div className="text-white">{booking.duration}</div></div>
                  <div><div className="text-sm text-gray-400 flex items-center gap-1"><MapPin className="h-3 w-3" />Location</div><div className="text-white">{booking.location}</div></div>
                  <div><div className="text-sm text-gray-400">Total Cost</div><div className="text-green-400 font-semibold">${booking.cost}</div></div>
                </div>
                <div className="flex items-center gap-3">
                  <Badge className={`${getStatusStyle(booking.status)} border`}>{booking.status}</Badge>
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button variant="outline" size="sm" className="border-white/20 text-white hover:bg-white/10 transition-all duration-300"><Eye className="h-4 w-4 mr-1" />Details</Button>
                    </DialogTrigger>
                    <DialogContent className="glass-card border-white/10 text-white">
                      <DialogHeader><DialogTitle>Booking Details</DialogTitle><DialogDescription className="text-gray-400">Booking #{booking.id}</DialogDescription></DialogHeader>
                  ...
                  {(booking.status === 'Pending' || booking.status === 'Confirmed') && (
                    <Button variant="outline" size="sm" className="border-red-500/30 text-red-400 hover:bg-red-500/10 transition-all duration-300"><X className="h-4 w-4 mr-1" />Cancel</Button>
                  )}
                </div>
              </div>
            </CardContent>
